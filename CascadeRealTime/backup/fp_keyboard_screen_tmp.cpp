...
void detect_keyboard(void *args){
    ...
    cascade.detectMultiScale(edges, detections, 1.1, 650, 0, cv::Size(160, 160));
...
void detect_mouse(void *args){
    ...
    std::vector<cv::Rect> detections;
    // image, obj, numDetections, scaleFactor, minNeighbors, flag, max_min size
    cascade.detectMultiScale(gray, detections, 1.1, 3, 0, cv::Size(100, 100));
...
int main(int argc, const char *argv[])
{
	...
	if (pthread_create(&t1, NULL, detect_keyboard, (void *)argst2) != 0) {
        cerr << "Error: pthreadt2_create\n";
    }
	if (pthread_create(&t1, NULL, detect_mouse, (void *)argst3) != 0) {
